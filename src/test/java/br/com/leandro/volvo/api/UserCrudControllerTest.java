...
public class UserCrudControllerTest {
	...
	public void shouldGetUser() throws Exception {
		Mockito.when(service.getById(Mockito.anyLong())).thenReturn(UserTestMass.buildUserResponse());

		//@formatter:off
		mockMvc.perform(get("/api/v1/user/1").headers(httpHeaders))
			...
			.andExpect(jsonPath("$.departmentId", Matchers.notNullValue()))
			;
		//@formatter:on
	...
	public void shouldGetAllUsers() throws Exception {
		Mockito.when(service.getAll()).thenReturn(UserTestMass.buildListUserResponse());

		//@formatter:off
		mockMvc.perform(get("/api/v1/user").headers(httpHeaders))
			.andExpect(status().isOk())
			.andExpect(content().contentTypeCompatibleWith(contentType))
			.andExpect(jsonPath("$", Matchers.notNullValue()))
			...
			.andExpect(jsonPath("$[0].departmentId", Matchers.notNullValue()))
			;
		//@formatter:on
	...
	public void shouldCreateUser() throws Exception {
		Mockito.when(service.create(Mockito.any())).thenReturn(UserTestMass.buildUserResponse());

		//@formatter:off
		mockMvc.perform(post("/api/v1/user").headers(httpHeaders)
				.content(UserTestMass.buildUserRequestAsString()))
	...
	public void shouldUpdateUser() throws Exception {
		Mockito.when(service.update(Mockito.anyLong(), Mockito.any()))
				.thenReturn(UserTestMass.buildUserResponse());

		//@formatter:off
		mockMvc.perform(put("/api/v1/user/1").headers(httpHeaders)
				.content(UserTestMass.buildUserRequestAsString()))
			.andExpect(status().isOk())
			.andExpect(content().contentTypeCompatibleWith(contentType))
			.andExpect(jsonPath("$", Matchers.notNullValue()))
	...
	public void shouldDeleteUser() throws Exception {
		...
		//@formatter:off
		mockMvc.perform(delete("/api/v1/user/1").headers(httpHeaders))
